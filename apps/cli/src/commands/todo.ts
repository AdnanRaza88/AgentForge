...
export const todoCommand: SlashCommand = {
  name: "todo",
  description:
    "List or manage session todos: /todo  |  /todo add <text>  |  /todo done <index>",
  async run(args, state) {
    ...
    if (!sub) {
      ...
      if (items.length === 0)
        return { output: chalk.gray("No todos yet. Use /todo add <text>") };
      const lines = items.map((t, i) => {
        const mark =
          t.status === "done" ? chalk.green("✓") : chalk.yellow("○");
    ...
    if (sub === "done") {
      ...
      if (isNaN(idx))
        return { output: chalk.red("Usage: /todo done <index>") };
